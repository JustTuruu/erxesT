...
} from '@/deals/graphql/mutations/BoardMutations';
import {
  GET_BOARDS,
  GET_BOARD_DETAIL,
} from '@/deals/graphql/queries/BoardsQueries';
...
export const useBoardAdd = (
  options?: MutationHookOptions<{ salesBoards: IBoard[] }>,
) => {
...
export const useBoardSaveAsTemplate = (
  options?: MutationHookOptions<{ salesBoardsSaveAsTemplate: any }>,
) => {
  const [saveBoardAsTemplate, { loading, error }] = useMutation(
    SAVE_BOARD_AS_TEMPLATE,
    {
      ...options,
      variables: {
      ...
      onCompleted: (...args) => {
        toast({
          title: 'Successfully saved board as template',
          variant: 'default',
        });
        options?.onCompleted?.(...args);
      },
